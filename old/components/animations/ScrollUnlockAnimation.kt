package com.promok.tech.old.components.animations
    ...
    LaunchedEffect(Unit) {
        val listener: (Event) -> Unit = { e ->
            ...
            if (we.deltaY > 0) { // negative deltaY means scrolling up
                scope.launch {
                    playAnimation = true
                    delay(_root_ide_package_.com.promok.tech.old.components.theme.AppTheme.Animations.scrollUnlockDelay) // match animation duration
                    onUnlock()
                    playAnimation = false
                }
            }
